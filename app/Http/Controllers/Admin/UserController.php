 ...
 public function viewUsers()
    {
      ...
      $users = User::where('users.role_id','!=',0)->orderby('users.pk_user_id','Desc')->get();	  
		...
		->addColumn('role', function ($row) {
            if ($row->role_id==2) {
                $role='<span class="badge rounded-pill bg-primary">User</span>';
            } 
			elseif ($row->role_id==3)
				{
                $role='<span class="badge rounded-pill bg-info">Sub User</span>';
            }
			else
				{
                $role='<span class="badge rounded-pill bg-success">Sup.Admin</span>';
            }
